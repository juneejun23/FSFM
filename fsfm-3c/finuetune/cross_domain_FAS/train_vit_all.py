...
from utils.utils_all import save_checkpoint, AverageMeter, Logger, accuracy, mkdirs, time_to_str
from utils.evaluate import eval
from utils.dataset_all import get_dataset
from fas import fas_model_fix

import random
import numpy as np
from config_all import configALL
...
def train(config, data_loader, args):
    # 0-shot: src1~4 학습, src5(celeb) 포함, tgt=all 테스트
    (src1_train_dataloader_fake, src1_train_dataloader_real,
     ...
     _, _, test_dataloader) = data_loader
    ...
    log.write('** start training target model (ALL: MCIO integrated) **\n', is_file=1)
    log.write(
    ...
    for iter_num in range(4000 + 1):
        ...
        if iter_num != 0 and (iter_num + 1) % iter_per_epoch == 0:
            ...
            save_checkpoint(
                save_list, is_best, net1,
                os.path.join(config.op_dir,
                             f'all_vit_0_shot_checkpoint_run_{str(config.run)}.pth.tar')
            )
            ...
            log.write(
                ...
                % ((iter_num + 1) / iter_per_epoch, valid_args[0], valid_args[6],
                   ...
                   time_to_str(timer() - start, 'min'), 0), is_file=1)
            log.write('\n', is_file=1)
