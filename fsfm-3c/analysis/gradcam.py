...
SAVE_DIR = '/home/junwoo/projects/FSFM/fsfm-3c/analysis/results/gradcam_fas_dlib'
...
# ── 개별 GradCAM++ 저장 ────────────────────────────────────
def save_gradcam(model, img_tensor, img_np, target_class, pred_class,
...
def plot_avg_gradcam_grid(all_class_data, save_path):
    ...
    for row_idx, cls_data in enumerate(all_class_data):
        ...
        maps_by_col = [cls_data['all_maps'], cls_data['correct_maps'], cls_data['wrong_maps']]
        axes[row_idx, 0].set_ylabel(cls_name, fontsize=11, fontweight='bold', labelpad=12)
    ...
    plt.suptitle(
        ...
        '(All Mean  /  Correct Mean  /  Misclassified Mean)',
        fontsize=14, fontweight='bold', y=1.01)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\n✅ 평균 GradCAM 그리드 저장: {save_path}")
...
if __name__ == '__main__':
    ...
    for cls_name, (img_dir, true_label, max_n) in SAMPLES.items():
        ...
        for sub in ['misclassified', 'correct']:
            os.makedirs(os.path.join(SAVE_DIR, cls_name, sub), exist_ok=True)

        # Step 1: 전체 예측값 계산 (detection 실패 제외)
        preds  = []
        n_skip = 0
        ...
        print(f"  Skip(det fail)={n_skip} | Valid={total} | Correct={n_correct} ({n_correct/total*100:.1f}%) | Misclassified={n_wrong} ({n_wrong/total*100:.1f}%)")

        all_results.append({
            'class': cls_name, 'skipped': n_skip, 'total': total,
            'correct': n_correct, 'misclassified': n_wrong,
            'accuracy': f'{n_correct/total*100:.1f}%',
            'miss_rate': f'{n_wrong/total*100:.1f}%',
        })

        # Step 2: 개별 GradCAM 저장 (최대 100장 랜덤)
        ...
        for i, p in enumerate(wrong_save):
            ...
            save_gradcam(model, p['img_tensor'], p['img_np'], p['pred_class'],
                         p['pred_class'], p['prob_real'], p['prob_fake'],
        ...
        all_class_data.append({
            'cls_name': cls_name, 'all_maps': all_maps,
            'correct_maps': correct_maps, 'wrong_maps': wrong_maps,
        })
        print(f"  Maps: 전체={len(all_maps)} / Correct={len(correct_maps)} / Misclassified={len(wrong_maps)}")

    # ── 통계 표 저장 ───────────────────────────────────────
    fig, ax = plt.subplots(figsize=(12, len(all_results) * 0.5 + 1.5))
    ax.axis('off')
    col_labels = ['class', 'skipped', 'total', 'correct', 'misclassified', 'accuracy', 'miss_rate']
    ...
    plt.title('FAS Model - Classification Statistics (DLIB crop)', fontsize=12, pad=10)
    plt.tight_layout()
